...
| コマンド | 説明 |
|---------|------|
| `/名前 MaiPlayer` | ユーザー名を MaiPlayer に設定 |
| `/名前 クリア` | ユーザー名を削除 |
| `/名前` | 現在のユーザー名と QQ 連携状態を確認 |
...
| コマンド | 説明 |
|---------|------|
| `/名前 123456789` | QQ 番号 123456789 を連携 |
| `/名前 qq解除` | QQ 番号の連携を解除 |
...
| コマンド | 説明 |
|---------|------|
| `/フレンド リスト` | 3 つのフレンドスロットを確認 |
| `/フレンド 追加 #ユーザー名` | ユーザー名でフレンド追加（最初の空きスロットに自動登録） |
| `/フレンド 追加 #ユーザー名 2` | 指定スロットに追加（既存を上書き） |
| `/フレンド 追加 #QQ番号` | QQ 番号でフレンド追加 |
...
::: tip 多言語対応
フレンドスロットは英語（`#friend1`）表記にも対応しています。
:::
...
::: warning 注意
PK を開始する前に、双方が B50 データを更新済みである必要があります（`/データ更新` または `/updateb50`）。また、相手の B50 プライバシー設定が公開になっている必要があります。
:::
