...
但使用者名稱畢竟可能不好記，或者會被修改——這時**好友系統**就派上用場了：將常用玩家新增為好友後，直接用 `#好友1` 即可查詢，無需記憶使用者名稱，且對方修改使用者名稱後也不受影響。
...
::: tip
使用者名稱是在 QQ 平台上讓他人查詢你、新增你的主要方式。如果你希望被群友查詢或 PK，建議優先設定使用者名稱。
:::
...
| 指令 | 說明 |
|------|------|
| `/使用者名稱 MaiPlayer` | 將使用者名稱設定為 MaiPlayer |
| `/使用者名稱 清除` | 刪除目前的使用者名稱 |
| `/使用者名稱` | 查看目前使用者名稱與 QQ 綁定狀態 |
...
- 其他用戶可以透過 `#QQ號碼` 查詢你或將你新增為好友，作為使用者名稱之外的備用方式
...
| 指令 | 說明 |
|------|------|
| `/使用者名稱 123456789` | 綁定 QQ 號碼 123456789 |
| `/使用者名稱 解綁qq` | 解除 QQ 號碼綁定 |
...
|------|------|
| `/好友 列表` | 查看目前 3 個好友槽位 |
| `/好友 新增 #使用者名稱` | 透過使用者名稱新增好友（自動填入第一個空槽） |
| `/好友 新增 #使用者名稱 2` | 新增好友到指定槽位（會覆蓋原有） |
| `/好友 新增 #QQ號碼` | 透過 QQ 號碼新增好友 |
| `/好友 刪除 1` | 刪除槽位 1 的好友 |

### 如何使用好友槽位查詢？

新增好友後，在任意支援指定查詢目標的指令中，用 `#好友N` 代替使用者名稱即可：
...
::: warning 注意
發起 PK 前，雙方均需已更新 B50 資料（`/更新b50` 或 `/更新記錄`），且對方的 B50 隱私權限需為公開。
:::
